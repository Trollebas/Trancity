...
//            var s = Directory.GetDirectories(path);
            foreach (var directory in Directory.GetDirectories(path))
            {
                ...
                {
                	Logging.Write("ModelLoader", "model.xml not found in directory " + directory1);
                    continue;
                }
                ...
                {
                	Logging.Write("ModelLoader", "Error in " + directory1 + "model.xml");
                    continue;
                }
            }
        }
...
//	        транспорт.количествоХвостов = (int)Xml.GetDouble(element3["tail_count"]);
	        ...
	        catch
	        {
	        	...
	        	Logging.Write("ModelLoader", "Bounding spheres not loaded for " + транспорт.name);
	        }
...
//			трамвай.расстояние_между_тележками = (double)Xml.GetDouble(element3["telegi_dist"]);
	        ...
	        catch
	        {
	        	...
	        	Logging.Write("ModelLoader", "Bounding spheres not loaded for " + трамвай.name);
	        }
        ...
        private static МодельТранспорта.АХ LoadStandaloneMotion(XmlNode node)
        {
            if (node != null)
            {
                ...
                return new МодельТранспорта.АХ(z, a);
            }
            return null;
        }
