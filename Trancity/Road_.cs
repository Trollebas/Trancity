...
namespace Trancity
{
    public class Road : MeshObject, MeshObject.IFromFile, IMatrixObject, IObjectContainer, ITest
    {
        ...
        public Road(double началоX, double началоY, double конецX, double конецY, double направление, bool прямая, double ширина0, double ширина1)
        {
            ...
            if (прямая)
            {
                направления[1] = направление + 3.1415926535897931;
            }
            else
            {
            ...
            while (направления[0] > 3.1415926535897931)
            {
                направления[0] -= 6.2831853071795862;
            }
            while (направления[0] <= -3.1415926535897931)
            {
                направления[0] += 6.2831853071795862;
            }
            while (направления[1] > 3.1415926535897931)
            {
                направления[1] -= 6.2831853071795862;
            }
            while (направления[1] <= -3.1415926535897931)
            {
                направления[1] += 6.2831853071795862;
        ...
        /*public void Test()
        {
        	...
        	for (int i = 0; i < matrices_count; i++)
        	{
        		matrixes[i] = Matrix.Translation(0.0f, 0.0f, 0.0f);
        	}
        	matrices_count = 1;
        }*/
...
//        	if ((!MainForm.in_editor) && (matrixes[index] != Matrix.Identity)) return matrixes[index];
        	if (!MainForm.in_editor) return matrixes[index];
            double num5;
            double num6;
            DoublePoint point;
            DoublePoint point2;
            ...
            else
            {
                ...
                num7 = НайтиНаправление(num2) + 3.1415926535897931;
        ...
        public DoublePoint НайтиКоординаты(double расстояние, double отклонение)
        {
            ...
            return (point + new DoublePoint(num += 1.5707963267948966) * отклонение);
        }
        ...
        public void НайтиПозицию(double расстояние, out DoublePoint координаты, out double направление)
        {
            if (кривая)
            {
                if (расстояние < структура.длина0)
                {
                    ...
                    {
                        направление = point.угол + 1.5707963267948966;
                    }
                    else
                    {
                        направление = point.угол - 1.5707963267948966;
                ...
                else
                {
                    ...
                    if (структура.радиус < 0.0)
                    {
                        направление = point2.угол + 1.5707963267948966;
                    }
                    else
                    {
                        направление = point2.угол - 1.5707963267948966;
        ...
        public void НайтиПозициюПоВысоте(double расстояние, out double нужнаяВысота, out double направлениеY)
        {
            ...
            {
                var num = Длина / 10.0;
                var point = new DoublePoint(Длина - (2.0 * num), высота[1] - высота[0]);
                var d = (Длина - (2.0 * num)) / point.модуль;
                ...
                else if (расстояние > ((Длина - num) - (num * d)))
                {
                    if (расстояние > Длина)
                    {
                        расстояние = Длина;
                    }
                    нужнаяВысота = высота[1] - (num5 * (num4 - Math.Sqrt((num4 * num4) - ((Длина - расстояние) * (Длина - расстояние)))));
                    направлениеY = num5 * Math.Asin((Длина - расстояние) / num4);
                }
                else
                {
        ...
        public virtual void ОбновитьСледующиеДороги(Road[] дороги)
        {
            ...
            foreach (var дорога in дороги)
            {
                ...
                var point = дорога.концы[0] - концы[1];
                if ((point.модуль < 0.01) && ((Math.Abs((дорога.направления[0] - направления[1]) + 3.1415926535897931) < 0.0001) || (Math.Abs((дорога.направления[0] - направления[1]) - 3.1415926535897931) < 0.0001)))
                {
                ...
                if ((point2.модуль < 0.01) && ((Math.Abs((дорога.направления[1] - направления[0]) + 3.1415926535897931) < 0.0001) || (Math.Abs((дорога.направления[1] - направления[0]) - 3.1415926535897931) < 0.0001)))
                {
                ...
                if ((point3.модуль < 0.01) && (((Math.Abs(дорога.направления[1] - направления[1]) < 0.0001) || (Math.Abs((дорога.направления[1] - направления[1]) + 6.2831853071795862) < 0.0001)) || (Math.Abs((дорога.направления[1] - направления[1]) - 6.2831853071795862) < 0.0001)))
                {
                    list3.Add(дорога);
                }
            }
        ...
        public Структура ОпределитьСтруктуру(bool index)
        {
            ...
            var point = концы[0];
            var point2 = концы[1];
            var angle = направления[0] + 1.5707963267948966;
            var num2 = направления[1] + 1.5707963267948966;
            var point3 = (new DoublePoint(angle) - new DoublePoint(num2)) / 2.0;
            var point4 = (point - point2) / 2.0;
            point3 /= new DoublePoint(point4.угол);
            var num3 = Math.Cos(Math.Asin(point3.y));
            if (index)
            {
                point5 = new DoublePoint(num3 - point3.x, 0.0) * new DoublePoint(point4.угол);
            }
            else
            {
                point5 = new DoublePoint(-num3 - point3.x, 0.0) * new DoublePoint(point4.угол);
            ...
            {
                point5 = point4 / point5;
                point3 *= new DoublePoint(point4.угол);
                определитьСтруктуру.радиус = point5.x;
                определитьСтруктуру.центр0 = point + (new DoublePoint(angle) * point5);
                определитьСтруктуру.центр1 = point2 + (new DoublePoint(num2) * point5);
                определитьСтруктуру.серединка = (определитьСтруктуру.центр0 + определитьСтруктуру.центр1) / 2.0;
                var point6 = (определитьСтруктуру.серединка - определитьСтруктуру.центр0) / (point - определитьСтруктуру.центр0);
                определитьСтруктуру.угол0 = (point6.угол + 6.2831853071795862) % 6.2831853071795862;
                var point7 = (point2 - определитьСтруктуру.центр1) / (определитьСтруктуру.серединка - определитьСтруктуру.центр1);
                определитьСтруктуру.угол1 = (point7.угол + 6.2831853071795862) % 6.2831853071795862;
                if (определитьСтруктуру.угол0 > (6.2831853071795862 - num4))
                {
                    определитьСтруктуру.угол0 -= 6.2831853071795862;
                }
                if (определитьСтруктуру.угол1 > (6.2831853071795862 - num4))
                {
                    определитьСтруктуру.угол1 -= 6.2831853071795862;
                }
        ...
        {
        	try{
			var v0 = НайтиВысоту(Длина / 2.0);
			var p0 = НайтиКоординаты(Длина / 2.0, 0.0);
        	var p2 = new Double3DPoint(p0.x, v0, p0.y);
        	base.bounding_sphere = new Sphere(p2, Длина / 2.0 + 5.0);}
        	catch {}
        }
