...
//        private Трамвай _первыйВагон;
        ...
        public override void АвтоматическиУправлять(World мир)
        {
        	...
        	if (передняя_ось.текущий_рельс.следующие_рельсы.Length > 0) рельс = передняя_ось.текущий_рельс.следующие_рельсы[передняя_ось.текущий_рельс.следующий_рельс];
        	else signals_dist = передняя_ось.текущий_рельс.Длина - передняя_ось.пройденное_расстояние_по_рельсу;
            ...
            foreach (var obj2 in передняя_ось.текущий_рельс.objects)
            {
            	if (obj2 is Stop)
            	{
	                ...
	                if ((((остановка.distance - передняя_ось.пройденное_расстояние_по_рельсу) <= 10.0) || //10.0
	                    ...
	                    ((маршрут == null) || (!остановка.typeOfTransport[маршрут.typeOfTransport]))) continue;// || (остановка!=nextStop)) continue;
            ...
            if (рельс != null)
            {
            	foreach (var obj3 in рельс.objects)
	            {
	            	if (obj3 is Stop)
	            	{
	...
	//	                    if ((!ось.передняя) && (!ось.задняя)) continue;
            	...
            	List<Положение> list2 = new List<Положение>(рельс.занятыеПоложения);
...
//                var list = new List<Положение>();//
                ...
                var point2 = Double3DPoint.Поворот(this.вагон.направление, MyFeatures.halfPI);
		    ...
		    public override void Обновить(World мир, Игрок[] игроки_в_игре)
            {
                ...
                if (list.Count > 0)
                {
					...
					foreach (Игрок игрок in list)
                    {
                        ...
                        {
                            DoublePoint point = игрок.cameraPosition.xz_point - this.хвосты[num3].position;
                        ...
                        {
                            DoublePoint point2 = игрок.cameraPosition.xz_point - this.сочленения2[num3].position;
                        ...
                        if (!flag)
                        {
                        	...
                        	var point3 = игрок.cameraPosition.xz_point - this.вагон.Координаты3D.xz_point;
                    ...
                    foreach (var игрок2 in list)
                    {
                        ...
                        pointArray[index].xz_point *= new DoublePoint(-иDArray[index].direction);
                        pointArray[index].xy_point *= new DoublePoint(-иDArray[index].НаправлениеY);
                    ...
                    foreach (Игрок игрок3 in list)
                    {
                        pointArray[index].xy_point *= new DoublePoint(иDArray[index].НаправлениеY);
                        pointArray[index].xz_point *= new DoublePoint(иDArray[index].direction);
                        pointArray[index] += иDArray[index].Координаты3D;
                        ...
                        игрок3.cameraPosition += pointArray3[index] - pointArray2[index];
                        if (игрок3.поворачиватьКамеру)
                        {
                            игрок3.cameraRotation += pointArray5[index] - pointArray4[index];
                        }
                        index++;
                    }
                }
...
//                var flag = false;
                ...
                for (int i = 0; i < тележки.Length; i++)
                {
                	if ((i < тележки.Length - 1) && (модель.тележки[i].index == модель.тележки[i+1].index))
                	{
                		...
                		dir[модель.тележки[i].index] = p.угол;
                		pos[модель.тележки[i].index] = тележки[i].координаты_3D + ((Double3DPoint)(new Double3DPoint(dir[модель.тележки[i].index]) * -модель.тележки[i].dist));
                		i++;
                		continue;
                	}
                ...
                if (тележки.Length == 1) dir[0] = тележки[0].координаты_3D.угол;
                ...
                for (int k = 0; k < this.сочленения2.Length; k++)
                {
                	сочленения2[k]._Координаты3D = pos[сочленения2[k]._index] + ((Double3DPoint)(new Double3DPoint(dir[сочленения2[k]._index]) * -сочленения2[k].dist));
                	...
                	if (сочленения2[k]._index > сочленения2[k]._target)
                	{
                		...
                		dir[сочленения2[k]._target] = p.угол;
                		pos[сочленения2[k]._target] += ((Double3DPoint)(new Double3DPoint(dir[сочленения2[k]._target]) * -x));
                	}
                	else
                	{
...
//                	сочленения2[k].координаты -= (Double3DPoint)(point3 * (axis_radius * 2));
        ...
        public class ЧастьТрамвая : MeshObject, IОбъектПривязки3D, IVector, IMatrixObject
        {
	        ...
	        /*public Тележка[] тележки = new Тележка[2];
	        //middle:
	        public int middle_index = -1;
	        public bool middle_front = false;*/
	        
	        /*public void УбратьТележку()
	        {
	        	var list = new List<Тележка>(тележки);
	        	list.RemoveAt(middle_front ? 0 : 1);
	        	тележки = list.ToArray();
	        }*/
...
//                    return (point2 != Double3DPoint.Zero.xz_point) ? (point + ((DoublePoint)((point2 / point2.модуль) * модель.расстояние_между_тележками))) : point;//модель.расстояние_между_тележками)));
					return вагон.Координаты3D + ((Double3DPoint)(new Double3DPoint(вагон.направление) * (модель.пантограф.pos.x - модель.пантограф.dist)));
                }
            }
...
//                    return "ось.x";
            ...
            public Double3DPoint координаты_3D
            {
                get
                {
                    ...
                    point.xz_point = position;
                    point.y = текущий_рельс.НайтиВысоту(пройденное_расстояние_по_рельсу) + 0.05;
                    double d = текущий_рельс.НайтиНаправлениеY(пройденное_расстояние_по_рельсу) + MyFeatures.halfPI;
...
//                	return "тележка.x";
            ...
            public double направление_y
            {
                get
                {
                    ...
                    DoublePoint point3 = new DoublePoint(point.xz_point.модуль, point.y);
                    return point3.угол;
                }
            }
        }
        
        public abstract class Токоприёмник_new
        {
...
//            public double lenght = 0.2;
            ...
            /*protected Matrix GetMatrix(Double3DPoint p1, Double3DPoint p2)
            {
                Double3DPoint point = p2 - p1;
                DoublePoint point2 = point.xz_point;//new DoublePoint(point.x, point.z);
                DoublePoint point3 = new DoublePoint(Math.Sqrt((point.x * point.x) + (point.z * point.z)), point.y);
                return ((Matrix.RotationY(-((float)point2.угол)) * Matrix.RotationAxis(new Vector3(-((float)point.z), 0f, (float)point.x), (float)point3.угол)) * Matrix.Translation((float)p1.x, (float)p1.y, (float)p1.z));
            }*/
            ...
            public void НайтиПровод(Контактный_провод[] контактныеПровода)
            {
                ...
                {
                	if (провод1.обесточенный)  continue;
                    var pos = position.xz_point;
                    var point = pos - провод1.начало;
                    point.угол -= провод1.направление;
                    if ((Math.Abs(point.y) <= width) && ((point.x >= 0.0) && (point.x <= провод1.длина)))
                    {
                    	DoublePoint point2 = провод1.начало + new DoublePoint(провод1.направление) * point.x;
                        DoublePoint point3 = point2 - pos;
                        num2 = провод1.найти_высоту(point.x);
                        if ((point3.модуль <= width) && (num2 < num))
                        {
                        	Провод = (Трамвайный_контактный_провод)провод1;
                        	num = num2;
                            continue;
                        }
                	}
            ...
            public double обычная_высота_max
            {
                get
                {
                	if (Провод != null)
                	{
                		return ((Провод.найти_высоту(ПройденноеРасстояниеПоПроводу) + Контактный_провод.высота_контактной_сети) - position.y - 0.03);
                	}
                	return высота_min;//(Рельс.высота_контактной_сети - 0.03);
                }
            }
            ...
            public double ПройденноеРасстояниеПоПроводу
            {
                get
                {
                    if (Провод != null)
                    {
                        var point = position.xz_point - Провод.начало;
                        return point.модуль;
                    }
                    return 0.0;
                }
            	...
            	public override void Обновить(World мир)
	            {
	                ...
	                if (Провод != null)
	                {
		                ...
		                else
		                {
		                	...
		                	var point = Провод.найти_координаты(dist, 0.0);
			                var point2 = position.xz_point - point;
			                if (point2.модуль > width)
			                {
			                	Провод = null;
