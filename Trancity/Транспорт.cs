...
//        public abstract void UpdateSound(Игрок[] игроки, bool игра_активна);
        ...
        protected void ОбновитьРейс()
        {
            ...
            if ((рейс_index != (рейс.pathes.Length - 1)) || (наряд == null))
            {
                ...
                if (рейс_index == рейс.pathes.Length)
                {
                    ...
                    {
                        if (текущее_положение.Дорога != рейс.pathes[i]) continue;
                        рейс_index = i;
                        break;
                    }
                }
            }
            if ((наряд == null) || (рейс_index != (рейс.pathes.Length - 1))) return;
            for (var j = 0; j < (наряд.рейсы.Length - 1); j++)
            {
                if (наряд.рейсы[j] != рейс) continue;
        ...
        {
        	if ((рейс == null) || ((рейс.tripStopList == null) || (рейс.tripStopList.Count == 0))) return;
...
//						stopIndex++;
        ...
        {
        	if ((рейс == null) || ((рейс.tripStopList == null) || (((nextStop == null) || ((stop == nextStop) || (stopIndex != 0)))))) return;
        	for (int i = 0; i < рейс.tripStopList.Count; i++)
        	{
        		if ((рейс.tripStopList[i].stop != stop) || (!рейс.tripStopList[i].flag)) continue;
        		stopIndex = i;
...
//        		MessageBox.Show("Couldn't create sound buffers for " + модель.name);
        		Logging.Write(модель.name, "Couldn't create sound buffers");
        		nosound = true;
        	}
        }
        ...
        {
        	if (!nosound) система_управления.UpdateSound(игроки, игра_активна);
        }
        
        public void LoadCameras()
        {
        	if (модель.cameras == null) return;
        ...
        {
        	if (index >= cameras.Length) return false;
