...
//		private static Size splash_size;
		...
		public static void Initialize()
		{
			...
			/*try
			{
				splash_tex = Texture.FromFile(MyDirect3D.device, "splash.png");
				using (Image b = Bitmap.FromFile("splash.png")) splash_size = b.Size;
			}
			catch
			{
				splash_tex = new Texture(MyDirect3D.device, 1, 1, 0, Usage.Dynamic, Format.A8R8G8B8, Pool.Default);
				splash_size = new Size(1, 1);
			}
			try
			{
				white_tex = Texture.FromFile(MyDirect3D.device, "white.bmp");
			}
			catch
			{
				white_tex = new Texture(MyDirect3D.device, 1, 1, 0, Usage.Dynamic, Format.A8R8G8B8, Pool.Default);
			}*/
		}
